...
|-------------|------|-----------|-------------|------|
| Open tab | 正在浏览 | `chrome.tabs` | Home 工作台 — 标签页与域名分组模块 | Phase 1 |
| Hibernate | 休眠中（仍在标签列表，但释放内存） | `chrome.tabs.discard()` | Home 工作台 — 休眠操作 | Phase 2 |
| Close tab | 刚关闭，可恢复 | `chrome.sessions` | Home 工作台 — 最近关闭模块 | Phase 1 |
| Read Later | 想看但还没看 | `chrome.readingList` | Home 工作台 — 稍后读模块 | Phase 3 |
| Bookmark | 长期收藏 | `chrome.bookmarks` | Bookmarks 视图 | Phase 1 |
| Top Sites | 高频访问 | `chrome.topSites` | Home 工作台 — 常用网站模块 | Phase 1 |
...
替代 Chrome 默认新标签页，作为整个扩展的主界面入口。页面采用 **首页工作台 + 书签视图 + 轻面板** 的产品框架：默认进入 Home 工作台，Bookmarks 作为次级整理视图，Health、Settings、Command Palette 都以 overlay/panel 形式打开，不作为一级主视图。

**布局结构：**

```
┌─────────────────────────────────────────────────────────────────┐
│  Brand      [Home] [Bookmarks]                Search...  Cmd+K  │
├─────────────────────────────────────────────────────────────────┤
│                    TabMark                                      │
...
- **HeaderBar**：顶部细导航，只保留品牌、Home/Bookmarks 切换、全局搜索入口。Health 和 Settings 不出现在主导航中。
...
- **主搜索入口 + 命令入口**：Cmd+K 打开 Command Palette，可搜索标签页、书签和快捷动作；Home Hero 搜索也应导向同一套搜索模型。
- **轻面板**：Health Panel、Settings Panel 由命令面板或显式按钮打开，覆盖在当前工作台之上，关闭后回到原上下文。
...
> Phase 1 仅实现规则 1（Chrome 分组）和规则 3（域名聚合），自定义规则在 Phase 3 的 Settings Panel 中实现。
...
Bookmarks 是保留的次级主视图，用于集中整理长期收藏。它不是 Home 的面板，也不是 Health/Settings 这类轻面板。

#### 4.3.1 书签浏览

- 顶部保留 HeaderBar 与全局搜索入口
- 页面主体使用书签专用整理布局：文件夹树、标签筛选、当前列表可以并排或在窄屏堆叠
- 当前文件夹下的书签列表是核心内容
...
搜索入口在 Bookmarks 视图下自动切换为书签搜索模式：
...
> 搜索入口在不同上下文下的行为：Home 默认优先搜打开的标签页与工作台内容，Bookmarks 默认搜书签，输入前缀 `@all` 或打开 Command Palette 可触发跨域统一搜索。
...
- 后续可通过 Command Palette 打开完整阅读列表面板，但不作为一级主视图

### 4.5 健康检查（Health Panel）（Phase 2）

健康检查是轻面板，不是一级页面。入口来自 Command Palette、Bookmarks 视图工具按钮，或 Settings Panel 的数据管理区。
...
- HeaderBar 的全局搜索按钮和 Home Hero 搜索共享同一搜索模型，Cmd+K 打开 Command Palette
- 搜索默认匹配当前上下文内容（Home 优先标签页/工作台模块，Bookmarks 优先书签）
- 输入前缀 `@all`、使用 Command Palette，或在空白上下文下搜索时，触发跨域统一搜索
- 结果分组展示：
  - 「已打开」→ 点击跳转到该标签页
  - 「已收藏」→ 点击新标签页打开
...
├── HeaderBar
│   ├── Brand / doodle mark
│   ├── Home / Bookmarks segmented switch
...
│   ├── 标签页 / 书签 / 稍后读 / 最近浏览搜索
│   └── 快捷动作：打开 Health Panel、Settings Panel、快照等
│
└── PanelModal（轻面板）
    ├── Health Panel：死链检测、重复检测、空文件夹检测
...
**订阅粒度原则：** 组件只订阅自己需要的字段。Home 工作台的标签页变化不触发 Bookmarks 视图重渲染，反之亦然。
...
- 新标签页替换 + HeaderBar + Home/Bookmarks 主视图切换 + Command Palette（搜索范围：tabs + bookmarks）
...
以下是需要设计的所有页面/视图。PRD 是唯一设计真源，工程 docs 只解释当前架构。

### 10.1 新标签页 — 整体框架

- 顶部是 HeaderBar：品牌、Home/Bookmarks 分段切换、全局搜索入口。
- 主视图集合只包含 `home | bookmarks`。`health` 和 `settings` 是 panel action，不是 route-level view。
- Home 是默认新标签页和主工作台；Bookmarks 是次级整理视图。
- Health Panel、Settings Panel 通过 Command Palette 或显式按钮打开。
- HeaderBar 在窄屏保持单行优先，必要时压缩计数和搜索占位，不引入独立主导航。
...
- 书签列表（列表模式 / 卡片模式可切换）是主内容。
- 文件夹树和标签云是筛选工具，可并排显示或在窄屏堆叠。
- 顶部操作栏：批量选择后出现（已选 N 项 + 删除 / 移动 / 加标签）。
- 每行书签：checkbox + favicon + 标题 + secondary metadata（域名、路径、日期、tags）+ 操作（加标签 / 删除）。
...
- 分组展示：
  ...
  - 「收藏的书签」（外链图标）
  - 「最近浏览」（Phase 3，时钟图标）
- 快捷动作包括打开 Health Panel、Settings Panel、保存/恢复快照。
- 每条结果：favicon + 标题 + URL 摘要 + 来源标签。
- 键盘导航：↑↓ 选择，Enter 打开，Esc 关闭。
- 无结果时提示"No results"。

### 10.5 Health Panel（Phase 2）

- PanelModal 内展示，不离开当前 Home/Bookmarks 上下文。
- 顶部：开始检查按钮 + 进度条。
- Tab 切换：死链 / 重复 / 空文件夹。
...
- 默认视图：新标签页默认展示 Home 还是 Bookmarks。
- 常用网站：显示/隐藏、数量（4/6/8）。
- 休眠策略：开启/关闭、空闲阈值（分钟）。
...
- 轻面板关闭后回到原上下文，不重置 Home/Bookmarks 的筛选与滚动位置
...
- HeaderBar 在窄屏下保持品牌、主视图切换、搜索入口三件事可用
